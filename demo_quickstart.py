...
STAGE_ORDER = ["TIS", "T0", "T1", "T1A", "T1B", "T2", "T2A", "T2B", "T3", "T4", "T4A", "T4B"]
...
class RunConfig:
    n_patients: int = 240
    metastasis_samples: int = 1000
    seed: int = 42
    output_dir: Path = field(default_factory=lambda: Path("outputs"))
    tcga_path: Path = field(default_factory=lambda: Path("data/tcga_2018_clinical_data.tsv"))
...
def survival_results_to_frame(results: SurvivalResults, cohort_label: str) -> pd.DataFrame:
    return pd.DataFrame(
        {
            "Cohort": [cohort_label],
...
def stage_code_to_numeric(code: str) -> float:
    ...
    value = STAGE_MAPPING.get(cleaned)
...
def analyse_survival(
    ...
    *,
    ...
    if treatment_label not in set(data["group"]) or control_label not in set(data["group"]):
        raise ValueError("Both treatment and control groups must be present in the dataframe.")
    ...
    for group in [control_label, treatment_label]:
        ...
        kmf.plot_survival_function(ci_show=True, ci_alpha=0.15, lw=2, color=color_lookup[group])
...
def plot_calibration_curves(
    ...
    *,
    ...
    for label, probs in probabilities.items():
        frac_pos, mean_pred = calibration_curve(y_test, probs, n_bins=10, strategy=strategy)
...
def prepare_tcga_survival_cohort(clinical_df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_stage = df["Neoplasm Disease Lymph Node Stage American Joint Committee on Cancer Code"].str.upper().str.strip()
    valid_n_stage = n_stage.isin({"N0", "N1", "N2", "N3", "N3A", "N3B"})
    group = np.where(n_stage == "N0", "Node-negative", "Node-positive")

    tumor_stage = df["American Joint Committee on Cancer Tumor Stage Code"].apply(stage_code_to_numeric)
...
def parse_cli_args(argv: Sequence[str] | None = None) -> RunConfig:
    ...
    return RunConfig(
        ...
        calibration_bin_strategy=args.calibration_strategy,  # type: ignore[arg-type]
...
def run(config: RunConfig | None = None) -> None:
    ...
    write_dataframe(
        survival_results_to_frame(survival_results, "Synthetic (Standard vs AI-guided)"),
        synthetic_summary_path,
    )

    metastasis_df = simulate_metastasis_dataset(n_samples=cfg.metastasis_samples, seed=cfg.seed)
    ...
    if cfg.skip_tcga:
        ...
        if tcga_path.exists():
            ...
            if not tcga_cohort.empty:
                ...
                tcga_group_summary = (
                    tcga_cohort.groupby("group")
                    .agg(
                        patients=("patient_id", "count"),
                        median_months=("time_months", "median"),
                        event_rate=("event", "mean"),
                    )
                    .round({"median_months": 1, "event_rate": 3})
                    .reset_index()
                )
            else:
                print(
    ...
    print(
        ...
        f"{synthetic_summary_path.resolve()}, {calibration_summary_path.resolve()}."
    )

    if tcga_results is not None and tcga_group_summary is not None:
        tcga_summary_path = output_dir / "tcga_survival_summary.csv"
        write_dataframe(
            survival_results_to_frame(tcga_results, "TCGA-STAD (Node-positive vs Node-negative)"),
            tcga_summary_path,
        )
